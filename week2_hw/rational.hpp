...
public:
   ...
   /// \details
   /// This operator*= multiplies a rational value with a rational variable.
   rational operator*=( const rational & rhs ){
      counter *= rhs.counter;
      denominator *= rhs.denominator;
      reduce();
   ...
   /* De << operator voor hw.
   friend std::ostream & operator<<(std::ostream& lhs, const rational & rhs){
       return lhs << rhs.counter << '/' << rhs.denominator<<std::endl
       << std::setw(4) << std::setfill('0') << rhs.counter<< "/" << std::setw(4) << std::setfill('0') << rhs.denominator<<std::endl
       << std::hex << rhs.counter << "/" << std::hex << rhs.denominator<<std::endl
       << "0x" << std::hex << rhs.counter<< "/" << "0x" << std::hex << rhs.denominator<<std::endl;
   }
    */
};
